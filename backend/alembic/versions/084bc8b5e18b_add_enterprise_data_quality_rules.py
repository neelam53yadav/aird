...
def upgrade() -> None:
    ...
    if not enum_exists('rulestatus'):
        op.execute("CREATE TYPE rulestatus AS ENUM ('DRAFT', 'ACTIVE', 'DEPRECATED', 'ARCHIVED')")
    if not enum_exists('ruleseverity'):
        op.execute("CREATE TYPE ruleseverity AS ENUM ('ERROR', 'WARNING', 'INFO')")
    if not enum_exists('auditaction'):
        op.execute("CREATE TYPE auditaction AS ENUM ('CREATE', 'UPDATE', 'DELETE', 'ACTIVATE', 'DEPRECATE', 'ARCHIVE')")
    ...
    if not table_exists('data_quality_rule_sets'):
            ...
            sa.Column('status', sa.Enum('DRAFT', 'ACTIVE', 'DEPRECATED', 'ARCHIVED', name='rulestatus', create_type=False), nullable=False),
    ...
    if not table_exists('data_quality_rules'):
        op.create_table('data_quality_rules',
            ...
            sa.Column('severity', sa.Enum('ERROR', 'WARNING', 'INFO', name='ruleseverity', create_type=False), nullable=False),
            sa.Column('status', sa.Enum('DRAFT', 'ACTIVE', 'DEPRECATED', 'ARCHIVED', name='rulestatus', create_type=False), nullable=False),
    ...
    if not table_exists('data_quality_rule_audit'):
        op.create_table('data_quality_rule_audit',
            ...
            sa.Column('action', sa.Enum('CREATE', 'UPDATE', 'DELETE', 'ACTIVATE', 'DEPRECATE', 'ARCHIVE', name='auditaction', create_type=False), nullable=False),
