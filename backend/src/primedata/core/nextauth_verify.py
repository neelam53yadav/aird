...
def verify_nextauth_token(token: str) -> Optional[Dict[str, Any]]:
    ...
    if not settings.NEXTAUTH_SECRET or settings.NEXTAUTH_SECRET == "REPLACE_WITH_64_CHAR_RANDOM_STRING_FOR_PRODUCTION_USE_ONLY":
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Token type detected - Parts: {num_parts}, Algorithm: {token_algorithm}, Encryption: {token_encryption}")
            ...
            if token_encryption or token_algorithm == "dir":
                ...
                try:
                    if num_parts == 4:
                        ...
                        ciphertext_tag_bytes = base64.urlsafe_b64decode(ciphertext_tag_part + '=' * (4 - len(ciphertext_tag_part) % 4))
                        ...
                        if len(ciphertext_tag_bytes) < tag_length:
                            logger.error(f"Ciphertext+tag too short: {len(ciphertext_tag_bytes)} bytes, need at least {tag_length}")
                            return None
                        
                        tag_bytes = ciphertext_tag_bytes[-tag_length:]
                        ciphertext_bytes = ciphertext_tag_bytes[:-tag_length]
                        
                        logger.info(f"Extracted - IV: {len(iv_bytes)} bytes, Ciphertext: {len(ciphertext_bytes)} bytes, Tag: {len(tag_bytes)} bytes")
                        ...
                        logger.info(f"JWE parts - Header: {len(jwe_header)}, Key: {len(encrypted_key)}, IV: {len(iv)}, Ciphertext: {len(ciphertext)}, Tag: {len(tag)}")
                        ...
                        try:
                            ...
                            logger.info(f"Decoded - IV: {len(iv_bytes)} bytes, Ciphertext: {len(ciphertext_bytes)} bytes, Tag: {len(tag_bytes)} bytes")
                                    ...
                                    if num_parts != 3:
                                        try:
                                            ...
                                            try:
                                                ...
                                                logger.error("")
                                                logger.error("SOLUTION: Clear browser cookies and sign in again with matching secrets")
        ...
        if settings.API_SESSION_EXCHANGE_ALLOWED_ISS:
            ...
            if token_iss != settings.API_SESSION_EXCHANGE_ALLOWED_ISS:
                logger.warning(f"Issuer mismatch. Token issuer: {token_iss}, Expected: {settings.API_SESSION_EXCHANGE_ALLOWED_ISS}")
                return None
        ...
        logger.error("Token signature is invalid. This usually means NEXTAUTH_SECRET doesn't match between frontend and backend.")
