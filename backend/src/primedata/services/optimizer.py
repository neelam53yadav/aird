...
def suggest_next_config(
    ...
    if trust_score < min_trust:
        ...
        suggestions.append(f"AI Trust Score ({trust_score:.1f}%) is good. Apply recommended improvements to push it to excellent (>85%) and maximize AI efficiency.")
    ...
    if quality < 70.0:
        ...
        suggestions.append(f"Quality score ({quality:.1f}%) is good but can be improved to excellent (>85%). Enable enhanced normalization and error correction to maximize AI efficiency.")
        actionable_recommendations.append({
            "type": "quality_normalization",
    ...
    if not policy_passed:
        ...
        if trust_score < 80.0:
            suggestions.append("Policy passed, but improving trust score above 80% would enhance data readiness and maximize AI application efficiency.")
            actionable_recommendations.append({
                "type": "excellence_push",
                ...
                "action": "apply_all_quality_improvements",
                "config": {"enhance_normalization": True, "error_correction": True, "extract_metadata": True, "increase_overlap": completeness < 80.0},
                "expected_impact": f"AI Trust Score improvement: +{(80.0 - trust_score):.1f}% to reach 80%+ (enhanced AI efficiency)",
                "priority": "high"
            })
        elif trust_score < 85.0:
            suggestions.append("Policy passed. Push AI Trust Score above 85% (excellent) to maximize AI application efficiency and achieve 4x productivity gains.")
            actionable_recommendations.append({
                "type": "excellence_push",
