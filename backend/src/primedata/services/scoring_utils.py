...
def score_context_quality(text: str) -> float:
    ...
    has_dates = bool(re.search(r'\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+\d{1,2},?\s+\d{4}\b|\b\d{1,2}[/-]\d{1,2}[/-]\d{2,4}\b|\b\d{4}\b', text_lower))
    ...
    contextual_indicators = [
        'because', 'therefore', 'however', 'although', 'in addition', 'furthermore',
        'specifically', 'for example', 'such as', 'including', 'namely', 'in particular',
        'according to', 'based on', 'related to', 'associated with', 'compared to',
        'as a result', 'consequently', 'meanwhile', 'furthermore', 'moreover'
    ]
...
def score_audience_intentionality(text: str) -> float:
    ...
    healthcare_terms = ["hcp", "physician", "patient", "regulatory", "doctor", "nurse", 
                        "clinician", "prescriber", "caregiver", "healthcare"]
    ...
    business_terms = ["executive", "management", "stakeholder", "board", "investor", 
                      "shareholder", "revenue", "profit", "quarterly", "annual"]
    ...
    tech_terms = ["developer", "engineer", "api", "sdk", "cli", "code", "implementation",
                  "integration", "deployment", "architecture", "technical"]
    ...
    ops_terms = ["operations", "monitoring", "maintenance", "support", "service", 
