...
from primedata.db.models import (
    Product, PipelineRun, PolicyStatus, RawFile, RawFileStatus,
...
def mark_raw_files_as_failed(product_id: UUID, version: int, error_message: str, db_session=None) -> int:
    ...
    try:
        raw_files_to_fail = db.query(RawFile).filter(
...
def register_stage_artifacts(
    ...
    if stage_name == "preprocess":
        ...
        csv_key = (
            result.artifacts.get('validation_summary_csv')
            if result.artifacts else None
        ) or result.metrics.get('validation_summary_path')
        if csv_key:
        ...
        pdf_key = (
            result.artifacts.get('trust_report_pdf')
            if result.artifacts else None
        ) or result.metrics.get('trust_report_path')
        if pdf_key:
            bucket = "primedata-exports"
            key = pdf_key
...
def get_dag_params(**context) -> Dict[str, Any]:
    ...
    logger.info(f"Extracted parameters: workspace_id={workspace_id}, product_id={product_id}, version={version}, playbook_id={playbook_id}, embedder={embedder_name}, dim={dim}")
    ...
    logger.info(f"Pipeline parameters validated: workspace_id={workspace_id}, product_id={product_id}, version={version}, embedder={embedder_name}, dim={dim}")
...
def task_preprocess(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Querying raw files for product_id={product_id} (type: {type(product_id).__name__}), version={version} (type: {type(version).__name__})")
        ...
        if all_product_files and not raw_file_records:
            ...
            logger.info(f"Pipeline is looking for version {version}, but files exist for versions: {sorted(versions_available)}")
        elif raw_file_records:
            # Log the actual files found for verification
            file_info = [f"{rf.filename} (stem: {rf.file_stem}, status: {rf.status.value}, minio_key: {rf.minio_key})" 
                        for rf in raw_file_records[:10]]  # Log first 10 files
            ...
            if mismatched:
                ...
                for rf in mismatched:
                    logger.error(f"  Mismatch: file_id={rf.id}, expected_product={query_product_id}, got_product={rf.product_id}, "
                               f"expected_version={query_version}, got_version={rf.version}")
                raise ValueError(f"Database integrity error: {len(mismatched)} raw files have mismatched product_id or version")
            else:
                logger.info(f"✓ All {len(raw_file_records)} raw files verified: correct product_id and version")
        
        if not raw_file_records:
            error_msg = f'No raw files found in database for product {product_id}, version {version}. Please run initial ingest first.'
        ...
        logger.info(f"Found {len(raw_files)} validated raw files in database to process (out of {len(raw_file_records)} total)")
        ...
        context['task_instance'].xcom_push(key='preprocess_result', value={
    ...
    try:
        ...
        if result.status == StageStatus.SUCCEEDED and aird_context.get('pipeline_run'):
            try:
                ...
                input_artifact_ids = [UUID(aid) for aid in preprocess_artifact_ids_str] if preprocess_artifact_ids_str else None
        ...
        context['task_instance'].xcom_push(key='scoring_result', value={
            'status': result.status.value,
            'metrics': result.metrics,
...
def task_fingerprint(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        # Store result in XCom (store before raising exception if failed)
        context['task_instance'].xcom_push(key='fingerprint_result', value={
            'status': result.status.value,
            'metrics': result.metrics,
    ...
    try:
        ...
        if result.status == StageStatus.SUCCEEDED and aird_context.get('pipeline_run'):
            try:
                ...
                input_artifact_ids = [UUID(aid) for aid in fingerprint_artifact_ids_str] if fingerprint_artifact_ids_str else None
                ...
                policy_artifact = PA(
                    ...
                    input_artifacts=[{
                        ...
                        "stage": "fingerprint",
                        "artifact_name": "fingerprint",
                    } for aid in input_artifact_ids] if input_artifact_ids else [],
                    artifact_metadata={
        ...
        if product and result.status == StageStatus.SUCCEEDED:
            ...
            std_logger.info(f"Updated product policy_status to {product.policy_status.value}, violations_count={len(violations)}")
...
def task_finalize(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        if pipeline_run_id:
            update_pipeline_status(pipeline_run_id, 'succeeded', {
        ...
        raw_files_to_finalize = db.query(RawFile).filter(
            RawFile.product_id == product_id,
            RawFile.version == version,
            RawFile.status == RawFileStatus.PROCESSING
        ).all()
        ...
        if raw_files_to_finalize:
            ...
            db.commit()
            logger.info(f"Marked {len(raw_files_to_finalize)} raw files as PROCESSED for product {product_id}, version {version}")
            std_logger.info(f"Marked {len(raw_files_to_finalize)} raw files as PROCESSED for product {product_id}, version {version}")
        else:
            ...
            try:
                ...
                if pipeline_run:
                    ...
                    if artifact_summary['total_artifacts'] == 0:
                        logger.warning(f"No artifacts found for pipeline_run_id={pipeline_run_uuid}, trying fallback query by product_id and version")
                        from primedata.db.models import PipelineArtifact, ArtifactStatus
                        fallback_artifacts = db.query(PipelineArtifact).filter(
                        ...
                        if fallback_artifacts:
                            logger.info(f"Found {len(fallback_artifacts)} artifacts via fallback query (product_id={product_id}, version={version})")
                    ...
                    logger.info(f"Artifact summary query: pipeline_run_id={pipeline_run_uuid}, found {artifact_summary['total_artifacts']} artifacts")
                    ...
                    logger.info(f"Added artifact summary to pipeline run metrics: {artifact_summary['total_artifacts']} artifacts, {artifact_summary['total_size_bytes']} bytes")
                    std_logger.info(f"Added artifact summary to pipeline run metrics: {artifact_summary['total_artifacts']} artifacts, {artifact_summary['total_size_bytes']} bytes")
                else:
        ...
        try:
            raw_files_in_error = db.query(RawFile).filter(
                RawFile.product_id == product_id,
                RawFile.version == version,
                RawFile.status == RawFileStatus.PROCESSING
            ).all()
