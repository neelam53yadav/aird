...
def preprocess(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        if not raw_files:
            ...
            return {
                'status': 'skipped',
                'message': 'No .txt files to process',
                'files_count': 0
            }
...
def validate_data_quality(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        report = asyncio.run(validator.validate_product_data(
            product_id=product_id,
            version=version,
...
def fingerprint(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Fingerprint generation completed: {result.status.value}, trust_score={result.metrics.get('trust_score', 0.0)}")
...
ingest_task >> preprocess_task >> score_task >> fingerprint_task >> policy_task >> [validation_task, reporting_task] >> chunk_task >> embed_task >> index_task >> validate_task >> validate_dq_task >> finalize_task
