...
class IndexingStage(AirdStage):
    ...
    def execute(self, context: Dict[str, Any]) -> StageResult:
        ...
        try:
            ...
            self.logger.info(
                f"📊 Total chunks to process: {total_chunks}, "
                f"average chunk length: {avg_chunk_length:.0f} characters"
            )
            ...
            for idx, (record_data, embedding) in enumerate(zip(all_records_data, all_embeddings)):
                ...
                stored_text = record_data["text"][:max_text_length] if len(record_data["text"]) > max_text_length else record_data["text"]
