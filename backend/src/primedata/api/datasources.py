...
async def create_datasource(
    ...
    if product.workspace_id != request_body.workspace_id:
        raise HTTPException(
            ...
            detail="Workspace ID does not match product's workspace"
        )
    
    # Check billing limits for data source creation
    current_datasource_count = db.query(DataSource).filter(
        DataSource.product_id == request_body.product_id
    ).count()
