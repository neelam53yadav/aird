...
class WebConnector(BaseConnector):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Default headers
        default_headers = {
            'User-Agent': 'PrimeData-WebConnector/1.0'
        }
    ...
    def sync_full(self, output_bucket: str, output_prefix: str) -> Dict[str, Any]:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if success:
                        ...
                        errors += 1
                        details['urls_failed'].append({
                            'url': url,
                            'error': 'Failed to store in MinIO'
                        })
        ...
        logger.info(f"Web sync completed: {files_processed} files, {bytes_transferred} bytes, {errors} errors in {duration:.2f}s")
        return result
