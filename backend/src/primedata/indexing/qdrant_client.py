...
class QdrantClient:
            ...
            for i in range(0, total_points, batch_size):
                ...
                for retry in range(max_retries):
                    try:
                        ...
                        logger.info(f"Upserted batch {batch_num}/{total_batches} ({len(batch)} points) to collection {collection_name}")
                        batch_success = True
                        break
                    except Exception as batch_error:
                        if retry < max_retries - 1:
                            wait_time = 2 ** retry  # Exponential backoff: 1s, 2s, 4s
                            logger.warning(f"Batch {batch_num}/{total_batches} failed (attempt {retry + 1}/{max_retries}): {batch_error}. Retrying in {wait_time}s...")
                            time.sleep(wait_time)
                        else:
                            logger.error(f"Failed to upsert batch {batch_num}/{total_batches} after {max_retries} attempts: {batch_error}")
                            return False
    ...
    def get_collection_name(self, workspace_id: str, product_id: str, version: int, product_name: Optional[str] = None, use_product_name: bool = True) -> str:
        """
    ...
    def find_collection_name(self, workspace_id: str, product_id: str, version: int, product_name: Optional[str] = None) -> Optional[str]:
        """
