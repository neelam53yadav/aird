...
class ContentAnalyzer:
    ...
    def analyze_content(self, content: str, filename: Optional[str] = None) -> ChunkingConfig:
        ...
        adjusted_config = self._adjust_for_content_characteristics(
            content, base_config, content_type
        )
    ...
    def _adjust_for_content_characteristics(self, content: str, base_config: Dict, content_type: ContentType) -> Dict:
        ...
        config['chunk_size'] = max(config['min_chunk_size'], 
                                 min(config['chunk_size'], config['max_chunk_size']))
    ...
    def _estimate_retrieval_quality(self, chunks: List[Dict], config: ChunkingConfig) -> str:
        ...
        if (config.min_chunk_size <= avg_size <= config.max_chunk_size and 
            size_variance < (avg_size * 0.3) ** 2):
            return "high"
